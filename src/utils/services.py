...
def get_udk(topic):
    ...
    completion: ChatCompletion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"{topic}.Ushbu dissertatsiya mavzusi bo'yicha UDKni aniqlab ber.Dissertatsiya hujjatlarida rasmiy yozilishi kerak bo‘lgan shaklida ber(javobning AI tomonidan generatsiya qilinganligi bo'lmasin).(faqat UDK kodi (fan shifrini bilan birga) va uning tarkibidagi qismlar tavsifini yoz va bundan boshqa hech narsa yozma)"
            }
        ],
    )
